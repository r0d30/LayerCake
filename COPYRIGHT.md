# LayerCake Copyright Information

## Overall Project Copyright
```
Copyright (c) 2024-2025 LayerCake Project
Original Creator: Rodrigue Noel (r0d30)
Licensed under MIT License with Enhanced Attribution

IMPORTANT: Commercial sale of LayerCake compiler, its libraries, 
or renamed forks is STRICTLY PROHIBITED without written permission.
Attribution required for all derived works.
```

## Component-Specific Copyrights

### Core Compiler Components
```
src/LC_core/ - Copyright (c) 2024-2025 Rodrigue Noel (r0d30)
├── lexer/       - Lexical analyzer and tokenizer
├── parser/      - Syntax parser and AST generator  
├── semantic/    - Semantic analyzer and type checker
├── optimizer/   - Code optimization engine
├── codegen/     - Assembly code generator
├── memory/      - Memory management system
├── errors/      - Error handling and reporting
└── utils/       - Utility functions and helpers
```

### Assembly Compiler Components
```
src/LC_asm/ - Copyright (c) 2024-2025 Rodrigue Noel (r0d30)
├── assembler/   - Assembly language processor
├── linker/      - Object file linker
├── optimizer/   - Assembly-level optimizations
└── targets/     - Platform-specific backends
```

### Tools and Utilities
```
cli/            - Command-line interface - Copyright (c) 2024-2025 Rodrigue Noel (r0d30)
scripts/        - Build and utility scripts - Copyright (c) 2024-2025 Rodrigue Noel (r0d30)
tests/          - Test suites and frameworks - Copyright (c) 2024-2025 Rodrigue Noel (r0d30)
docs/           - Documentation and guides - Copyright (c) 2024-2025 Rodrigue Noel (r0d30)
```

### Configuration Files
```
Makefile        - Build configuration - Copyright (c) 2024-2025 Rodrigue Noel (r0d30)
.vscode/        - IDE configuration - Copyright (c) 2024-2025 Rodrigue Noel (r0d30)
```

## Third-Party Components
*Currently none - all code is original*

## File Header Template
All source files should include this header:
```assembly
;===============================================================================
; LayerCake Compiler - [Component Name]
; Copyright (c) 2024-2025 LayerCake Project
; Original Creator: Rodrigue Noel (r0d30) <noel.rodrigue.7850@gmail.com>
; Licensed under MIT License with Enhanced Attribution
; 
; This file is part of the LayerCake programming language project.
; See LICENSE file for full license terms.
;===============================================================================
```

## Documentation Copyright
```
All documentation, including:
- README files
- Technical guides  
- API documentation
- User manuals
- Example code
- Tutorial materials

Copyright (c) 2024-2025 Rodrigue Noel (r0d30)
Licensed under Creative Commons Attribution 4.0 International License
```

## Trademark Information
```
"LayerCake" and related marks are trademarks of Rodrigue Noel (r0d30)
Commercial sale of LayerCake compiler or libraries is PROHIBITED
Renaming LayerCake to avoid attribution is PROHIBITED
Usage requires proper attribution and compliance with license terms
See LICENSE file for detailed restrictions and commercial rights
```

## Commercial Rights Summary
- **Original Creator**: Retains all commercial licensing rights for LayerCake
- **Contributors**: Credited for contributions, bound by license terms
- **Users**: Free to use, modify, and distribute under license terms with attribution
- **Commercial Restriction**: Selling LayerCake compiler, libraries, or renamed forks is PROHIBITED
- **Attribution Required**: All derived works must credit LayerCake and retain copyright notices
- **No Renaming**: Creating renamed copies of LayerCake to avoid attribution is PROHIBITED

## Legal Contact
For copyright, licensing, or legal questions:
- Email: noel.rodrigue.7850@gmail.com
- Business: noel.rodrigue.7850@gmail.com

---
*This copyright information is binding and supersedes any conflicting statements elsewhere in the project.*
